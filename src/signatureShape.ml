...
module TypeShape = struct
  type t = {
    nb_function_parameters : int;
    nb_type_applications_result :int;
  }
  ...
  let rec get_nb_function_parameters (typ : Types.type_expr)
    ...
    | Tarrow (_, _, typ2, _) ->
      let (nb_function_parameters, result_typ) =
        get_nb_function_parameters typ2 in
...
let of_signature_without_hash
  (attributes : Typedtree.attributes option) (signature : Types.signature) : t =
  let shape_list =
    signature |> Util.List.filter_map (fun item ->
      ...
      | Sig_module (ident, _, _, _, _) -> Some (Name.of_ident_raw ident, None)
      | _ -> None
    ) in
  let high_level =
    shape_list |>
  ...
  let compute_precise =
    ...
    | None -> true
    | Some attributes -> Attribute.has_precise_signature attributes in
  let precise =
    if compute_precise then
      Some (
        shape_list |>
        List.to_seq |>
        Name.Map.of_seq
...
let of_signature
  (attributes : Parsetree.attributes option) (signature : Types.signature) : t =
