...
let rec of_ocaml_desc (module_typ_desc : Typedtree.module_type_desc)
  : t Monad.t =
  match module_typ_desc with
  | Tmty_alias _ ->
    raise
  ...
  | Tmty_functor (Named (ident, _, param), result) ->
    ...
    let* param =
      ...
      | _ :: _ ->
        raise
          (Module.Error "functor_parameter")
          NotSupported
          (
  ...
  | Tmty_typeof _ ->
    raise
      ([], Module.Error "typeof")
      NotSupported
      "The typeof in module types is not handled"
...
let rec to_typ (functor_params : Name.t list) (module_name : string)
  ...
  let* new_functor_params =
    params |>
    Monad.List.map (fun (name, _) -> Name.of_string false name) in
