...
Definition h {A : Type} (x : A) : Z :=
  match x with
  | _ => OCaml.List.hd l2
  end.

Definition t {A : Type} (x : A) : list Z :=
  match x with
  | _ => OCaml.List.tl l2
  end.

Definition x {A : Type} (x : A) : Z :=
  match x with
...
Definition asso {A : Type} (x_1 : A) : string :=
  match x_1 with
...
Definition com {A : Type} (x_1 : A) : list (Z * Z) :=
  match x_1 with
  | _ => OCaml.List.combine l2 l2
  end.

Definition so {A : Type} (x_1 : A) : list Z :=
  match x_1 with
  | _ => OCaml.List.sort (fun x => fun y => Z.sub y x) l2
  end.

Definition sso {A : Type} (x_1 : A) : list Z :=
  match x_1 with
  | _ => OCaml.List.stable_sort (fun x => fun y => Z.sub y x) l2
  end.

Definition fso {A : Type} (x_1 : A) : list Z :=
  match x_1 with
